...
den_fishnet <- st_read("Data_Denver/den_fishnet/den_fishnet.shp") %>% st_as_sf()

den_fishnet$ID_FISHNET <-as.character(den_fishnet$ID_FISHNET)

```

```{r, load boundary for denver}
...
```{r, select fishnet cells by centroid for denver}
# Centroid-in-polygon join to see which cells have their centroid in the denver boundary
den_selectCentroids <-
...
```{r}
#inundationModel <- glm(inundation ~ ., family="binomial"(link="logit"),
#                  data=(calgaryTrain) %>% as.data.frame() %>% dplyr::select(-ID_FISHNET, -geometry))
