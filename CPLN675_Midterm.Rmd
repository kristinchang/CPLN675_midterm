...
_Why it was chosen as a feature to explore (esp why this is needed in addition to mere distance to stream ー frequency of intersections with features could indicate damper soils, etc)_
* To calculate the number of intersections with existing hydrological features per fishnet grid cell, the lengths(st_intersects) functionality in R is used to create a count feature variable, **`n_hydro_int`**.
