...
map.elevation <- ggplot() +
  ...
  scale_fill_viridis()+
  labs(title = "Elevation (Mean)", fill="Elevation \n(meters)") + mapTheme()
...
map.flowaccmax <- ggplot() +
  ...
  labs(title = "Flow Accumulation (Max)", fill="Pixels \n") + mapTheme()
...
map.hydroints <- ggplot() +
  ...
  scale_fill_viridis()+
  labs(title = "Hydrology Intersections", fill="Number \n") + mapTheme()
```
...
grid.arrange(map.elevation, map.diststreams, map.flowaccmax, map.steepslopedist, map.distparks, map.hydroints,
             ncol=2,
             top = "Selected Significant Features included in Model")
```
...
------------- | ------------- 
True Positive (TP) | Model predicts inundation and there is inundation
False Positive (FP| Model predicts inundation and there is no inundation
True Negative (TN) | Model predicts no inundation and there is no inundation
False Negative (FN) | Model predicts no inundation and there is inundation

The frequency of each scenario is displayed in the confusion matrix below. These results support our above observation that the model predicts well for areas of no inundation than areas of inundation.  
...
This second map shows the predicted probabilities, but with the 2013 flood extent overlay. This helps to compare the predicted inundation outcomes (towards the green and yellow part of the gradient) with the actual data used to train the model in the first place.
